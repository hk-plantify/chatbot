...
def question_to_sql(user_question: str) -> str:
    messages = [
        {
            "role": "system",
            "content": "당신은 데이터베이스 전문가이며 MySQL 쿼리 생성기로 동작합니다. \
        ...
        },
        {
            ...
            작업 요구 사항:
            1. 질문이 개인화된 경우, 적절한 조건(user_id)을 포함하세요.
            2. 질문이 개인화되지 않은 경우, user_id 조건을 제외하고 일반적인 조건으로 쿼리를 작성하세요.
            3. 반환된 쿼리는 반드시 SELECT로 시작해야 합니다.
            """
        }
    ]
    ...
    function_call = {
        ...
        "parameters": {
            "type": "object",
            "properties": {
                "sql_query": {"type": "string", "description": "생성된 SQL 쿼리"}
            },
            "required": ["sql_query"]
        }
    }

    response = sql_llm(messages, function_call=function_call)

    # Extract the SQL query from the function call response
    if "sql_query" not in response["choices"][0]["message"]["function_call"]["arguments"]:
        raise ValueError("유효한 SELECT SQL 쿼리가 반환되지 않았습니다.")

    sql_query = response["choices"][0]["message"]["function_call"]["arguments"]["sql_query"]
